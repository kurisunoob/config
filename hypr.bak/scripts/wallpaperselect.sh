...
r_override="element{border-radius:${elem_border}px;} listview{columns:6;lines:2;spacing:10px;} element{padding:0px;orientation:vertical;} element-icon{size:${x_monres}px;border-radius:0px;} element-text{padding:20px;}"
# launch rofi menu
...
done | rofi -show-icons -dmenu -theme-str "${r_override}" -config "${RofiConf}" -select "${currentWall}")
